...
public class UniqueCombinationNthTest {
    ...
    public void assertCount(){
        ...
        for(int n=3; n<6; n++) {
            ...
            for(int increment=1; increment<=4; increment++) {
                ...
                long size = JNumberTools.combinations().of(input,combinationSize).uniqueNth(increment).stream().count();
    ...
    public void shouldReturnSameResultForDifferentIteratorObjects(){
        ...
        var iterable = JNumberTools.combinations().of(input,2).uniqueNth(2);
    ...
    private String getResultViaDirectIncrement(List<String> input, int r, int increment) {
        return JNumberTools.combinations().of(input,r)
                .uniqueNth(increment)
                .stream().toList().toString();
    }

    private String getExpectedResultViaOneByOneIteration(List<String> input, int r, int increment) {
        var stream = JNumberTools.combinations().of(input,r).unique().stream();
