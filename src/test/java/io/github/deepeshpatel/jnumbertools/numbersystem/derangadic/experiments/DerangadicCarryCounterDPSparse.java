...
/**
 * Memory-optimized Derangadic Carry Counter using sparse DP + flattened legal table.
 * CORRECTED: Proper dependency ordering (nextMask has more bits, so process by decreasing bitCount).
 */
public final class DerangadicCarryCounterDPSparse {
    ...
    // ===================================================================
    // Core Computation (CORRECTED SPARSE DP)
    // ===================================================================
    ...
    private BigInteger withinLayerCount(int m, int L) {
        ...
        // SPARSE: Use HashMap, but process in correct dependency order
        // nextMask = mask | (1<<chosen) has MORE bits than mask
        // So we must process masks in DECREASING bitCount order
        Map<Integer, BigInteger> memo = new HashMap<>();
        memo.put(maxMask - 1, BigInteger.ONE);  // base case: all positions placed

        // Process by decreasing bitCount (more bits first)
        for (int bits = m - 1; bits >= 0; bits--) {
            for (int mask = 0; mask < maxMask; mask++) {
                if (Integer.bitCount(mask) != bits) continue;
                ...
                for (int d = dStart; d <= dEnd; d++) {
                    ...
                    sum = sum.add(memo.getOrDefault(nextMask, BigInteger.ZERO));
                }
                memo.put(mask, sum);
            }
        }

        return memo.getOrDefault(0, BigInteger.ZERO);
    }
    ...
    // ===================================================================
    // Flattened Legal Table (unchanged from original)
    // ===================================================================
    ...
    // ===================================================================
    // Main (same validation as original)
    // ===================================================================

    public static void main(String[] args) {
        var counter = new DerangadicCarryCounterDPSparse();
